...
export function HeroSlider() {
  ...
  return (
    <div className="grid gap-5">
      <div className="relative min-h-[390px] overflow-hidden rounded-[2rem] bg-[#071827] soft-shadow lg:min-h-[460px]">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#071827]/82 via-[#071827]/16 to-transparent" />

        <div className="absolute left-4 right-4 top-4 flex items-center justify-between">
          <div className="rounded-full bg-white/92 px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-[#0877c9] shadow-lg backdrop-blur">
            Commercial cleaning
          </div>
          <div className="flex gap-2">
            <button
              aria-label="Previous cleaning image"
              className="grid size-10 place-items-center rounded-full border border-white/28 bg-white/88 text-[#071827] shadow-lg backdrop-blur transition hover:bg-white"
        ...
        <div className="absolute inset-x-4 bottom-4 rounded-3xl border border-white/18 bg-white/94 p-5 shadow-xl backdrop-blur">
          <p className="text-lg font-black text-[#071827]">{currentSlide.title}</p>
            ...
            {heroSlides.map((slide, index) => (
              <button
                ...
                className={`h-2.5 rounded-full transition ${
                  active === index ? "w-9 bg-[#ec1f72]" : "w-2.5 bg-slate-300 hover:bg-[#0877c9]"
                }`}
                key={slide.title}
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {heroServiceTiles.map((tile) => (
          <article className="group relative min-h-40 overflow-hidden rounded-[1.4rem] bg-[#071827] p-5 text-white shadow-sm" key={tile.title}>
            <img
              alt={tile.title}
              ...
              src={tile.image}
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#071827]/92 via-[#071827]/54 to-transparent" />
